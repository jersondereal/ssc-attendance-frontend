  ...
  return (
    <div className="flex flex-row gap-2">
      {CATEGORIES.map((cat) => (
        <div
          key={cat.id}
          className="border-r border-border-dark last:border-r-0 pr-2 last:pr-0 md:border-none md:pr-0"
        >
          <button
            type="button"
            onClick={() => onToggle(cat.id)}
            className="flex items-center gap-1.5 md:px-3 md:py-1.5 rounded-[8px] md:border border-gray-300 md:bg-white md:hover:bg-gray-100 text-sm text-gray-500 font-medium transition-colors cursor-pointer"
          >
...
const IMAGE_URL_RE =
  /!\[([^\]]*)\]\((https?:\/\/[^)]+)\)|(https?:\/\/\S+\.(?:png|jpg|jpeg|gif|webp)(\?\S*)?)/gi;
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith("`") && part.endsWith("`")) {
      return (
        <code
          key={i}
          className="bg-gray-100 border border-gray-300 rounded px-1 py-0.5 text-xs font-mono"
        >
          {part.slice(1, -1)}
        </code>
      );
  ...
  while ((match = IMAGE_URL_RE.exec(line)) !== null) {
    ...
    result.push(
      <img
        ...
        src={src}
        alt={match[1] || "image"}
        className="mt-2 rounded-[8px] max-w-[160px] max-h-[160px] object-cover border border-gray-300"
      />,
    );
  ...
  while (i < lines.length) {
    ...
    if (/^\|/.test(line)) {
      ...
      const dataRows = rows.filter(
        (_, idx) => idx !== 0 && !isSeparator(rows[idx]),
      );
      elements.push(
            ...
            <thead>
              <tr>
                {headerRow.map((cell, j) => (
                  <th
                    key={j}
                    className="border border-gray-300 px-3 py-1.5 text-left font-semibold bg-gray-50 whitespace-nowrap"
                  >
                    {renderInline(cell)}
                  </th>
                ))}
            ...
            <tbody>
              {dataRows.map((row, j) => (
                <tr key={j} className="even:bg-gray-50">
                  {row.map((cell, k) => (
                    <td key={k} className="border border-gray-300 px-3 py-1.5">
                      {renderLineWithImages(cell)}
                    </td>
                  ))}
                </tr>
              ))}
      ...
      elements.push(
        <pre
          key={`code-${i}`}
          className="bg-gray-50 border border-gray-300 rounded-[8px] p-3 text-xs overflow-x-auto my-2 font-mono text-gray-800"
        >
          <code>{codeLines.join("\n")}</code>
        </pre>,
      );
      ...
      if (textItems.length > 0) {
        elements.push(
          <ul
            key={`ul-${i}`}
            className="list-disc list-inside my-1 space-y-0.5 text-sm text-gray-800"
          >
            {textItems.map((item, j) => (
              <li key={j}>{renderLineWithImages(item)}</li>
            ))}
          </ul>,
        );
      }
      imageItems.forEach((item, j) => {
        elements.push(
          <div key={`ulimg-${i}-${j}`} className="mt-1">
            {renderLineWithImages(item)}
          </div>,
        );
      });
      continue;
      ...
      elements.push(
        <ol
          key={`ol-${i}`}
          className="list-decimal list-inside my-1 space-y-0.5 text-sm text-gray-800"
        >
          {items.map((item, j) => (
            <li key={j}>{renderLineWithImages(item)}</li>
          ))}
      ...
      elements.push(
        <p
          key={`h3-${i}`}
          className="font-semibold text-sm text-gray-900 mt-2 mb-0.5"
        >
      ...
      elements.push(
        <p
          key={`h1-${i}`}
          className="font-semibold text-base text-gray-900 mt-2 mb-0.5"
        >
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 h-full bg-white">
        ...
        <div className="w-full max-w-4xl mx-auto">
          <div className="space-y-1 pb-5">
            ...
            <p className="text-sm text-gray-500">
              Ask anything about attendance, events, students and fines
            </p>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 md:left-60 bg-transparent border-border-dark pb-6 px-5 md:px-10 z-20">
        ...
        <div
          ref={chatboxRef}
          className="relative w-full max-w-[920px] mx-auto border border-gray-300 rounded-[8px] bg-white focus-within:border-border-focus focus-within:ring-2 focus-within:ring-zinc-200 transition-all px-3 pt-3 pb-2 cursor-text"
          onClick={() => inputRef.current?.focus()}
        >
          {/* Dropdown — full chatbox width, opens upward */}
          <div
            className={`absolute z-50 px-2 bottom-full left-0 right-0 mb-1.5 bg-white border border-gray-300 rounded-[8px] shadow-sm overflow-hidden
          ...
          >
            {CATEGORIES.find((c) => c.id === openCategoryId)?.prompts.map(
              (prompt, idx) => (
                <button
                  key={idx}
                  type="button"
